...
class TestPollingDataStreamer(unittest.TestCase):
    def setUp(self):
        ...
        self.streamer = PollingDataStreamer(
            self.mock_data_manager, Mock(), self.mock_concurrency_manager
        )

    def test_start_streaming(self):
        self.mock_concurrency_manager.submit_job = Mock()
        self.streamer.start_streaming()
        self.mock_concurrency_manager.submit_job.assert_called_once_with(
            self.streamer.streaming_worker
        )
