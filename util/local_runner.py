...
def main() -> None:
    init()
    print(Fore.BLUE + "Running..." + Style.RESET_ALL)
...
def install_dependancies() -> None:
    print(Fore.BLUE + "Installing Dependancies..." + Style.RESET_ALL)
    run_command(f"pip install -r \"{os.path.join(AI_API_DIRECTORY, 'requirements.txt')}\"")
    print(Fore.GREEN + "Dependancies Installed!" + Style.RESET_ALL)


def run_ai_api() -> None:
    install_dependancies()
    print(Fore.GREEN + "AI API URL: http://127.0.0.1:5000" + Style.RESET_ALL)
    run_command(f"python \"{os.path.join(AI_API_DIRECTORY, 'app.py')}\"")
